...
int main()
{
    ...
    cout<<"Enter the arrival time, the burst time and priority in this respective order for each process:\n";
    int a,b,p;
   ...
   /*sort(readyQueue.begin(),readyQueue.end(),[](pcb const& lhs, pcb const& rhs) {
    ...
    for(int i=1;i<=n;i++)
    {
        cout<<"P"<<i<<":\t\t "<<readyQueue[i-1].arrivalTime<<"\t\t\t"<<readyQueue[i-1].burstTime<<"\t\t\t"<<readyQueue[i-1].priority<<"\t\t\t"<<readyQueue[i-1].waitingTime<<"\t\t\t"<<readyQueue[i-1].turnAroundTime<<endl;
